...
interface WipRow {
  ...
  original_contract_value: number; approved_changes: number; revised_contract_value: number;
  costs_to_date: number; billings_to_date: number; projected_profit: number;
}
...
export default function CompanyOverview() {
  ...
  const totalBilled = wipData.reduce((s, w) => s + (w.billings_to_date || 0), 0)
  const totalProfit = wipData.reduce((s, w) => s + (w.projected_profit || 0), 0)
  const companyMargin = totalContractValue > 0 ? (totalProfit / totalContractValue) * 100 : 0
    ...
    <div className="space-y-6 max-w-[1400px]">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-5 gap-4">
        ...
        <StatCard
          ...
          icon={TrendingUp}
          subtitle={hasProjectData ? formatCurrency(totalProfit) + ' projected profit' : undefined}
          ...
          {!hasProjectData ? (
            ...
            <div className="overflow-x-auto">
              <table className="w-full text-sm">
                ...
                <tbody>
                  {wipData.map((row) => {
                    const overUnder = (row.costs_to_date > 0 && row.revised_contract_value > 0)
                      ? ((row.costs_to_date / (row.costs_to_date + (row.revised_contract_value - row.costs_to_date - row.projected_profit))) * row.revised_contract_value) - row.billings_to_date
                      : 0
                    const margin = row.revised_contract_value > 0
                      ? (row.projected_profit / row.revised_contract_value) * 100
                      : 0
                    return (
                      <tr key={row.project_id} className="border-b border-gray-100 hover:bg-gray-50">
                        <td className="py-2.5 pr-4">
                          ...
                          {row.code && <p className="text-xs text-gray-500">{row.name}</p>}
                        </td>
                        <td className="py-2.5 px-3 text-right font-medium">{formatCurrency(row.revised_contract_value)}</td>
                        <td className="py-2.5 px-3 text-right">{formatCurrency(row.billings_to_date)}</td>
